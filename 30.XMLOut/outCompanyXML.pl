...
while (my $row = <$fh>) {
    ...
    my $inner_dep_nm = decode('UTF-8', $department_nm);

    next if $company_cd ne $company_cd1;
    next if $department_nm eq "" || 
	    $inner_dep_nm eq "出向他" || 
	    $inner_dep_nm eq "過去支店" || 
	    $inner_dep_nm eq "療養支店長" ||
	    $inner_dep_nm eq "(療養支店長)" ||
	    $department_cd1 eq "12102000" ||
	    $department_cd1 eq "12102100" ||
	    $department_cd1 eq "12102200" ||
	    $department_cd1 eq "12102300";
    ...
    # 親子関係を溜め込む
    unless ( exists $relations{$department_cd1} ) {
	$relations{$department_cd1} = $parent_cd;
    }
...
#   open my $fh2, "<:encoding(shiftjis)", $file_syozoku or die qq/Can't open file $file_syozoku : $!/;
	...
	if ( exists $post_hen_list{$post_cd} ) {
	    $post_cd = $post_hen_list{$post_cd}; 
	}
...
foreach my $department_cd (sort keys %relations) {
    my $parent_cd = $relations{$department_cd};
    unless ( $department_cd ne $parent_cd && $parent_cd eq "" ) {
...
while (my $row = <$fh_post>) {
    ...
    # 役職コード, 役職名, ランク, ソート
    my ($post_cd, $post_nm, $rank, $sort) = split(/\,/, $row);
